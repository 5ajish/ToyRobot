...
- Extract the following key configurations:
  ...
  - `prd.featureFilePattern`: Pattern for feature files (e.g., `feature-{n}.{n}*.md`)
...
#### 1.1 Locate Epic and Feature Files
...
- Look for feature files using `featureFilePattern` in the same location as epics
...
- **If a highest story file exists (`{lastEpicNum}.{lastFeatureNum}.{lastStoryNum}.story.md`):**
  - Verify its `Status` is 'Done' (or equivalent).
  - If not 'Done', present an alert to the user:

    ```plaintext
    ALERT: Found incomplete story:
    File: {lastEpicNum}.{lastFeatureNum}.{lastStoryNum}.story.md
  ...
  - If proceeding: Look for the Feature File for `{lastEpicNum}.{lastFeatureNum}` (e.g., `feature-{lastEpicNum}.{lastFeatureNum}*.md`) and parse it to find ALL stories in that feature. **ALWAYS select the next sequential story** (e.g., if last was 2.2.3, next MUST be 2.2.4).
  - If the next sequential story has unmet prerequisites, present this to the user:

    ```plaintext
    ALERT: Next story has unmet prerequisites:
    Story: {epicNum}.{featureNum}.{storyNum} - {Story Title}
  ...
  - If there are no more stories in the current feature (e.g., 2.2.9 was done and there is no 2.2.10):

    ```plaintext
    Feature {epicNum}.{featureNum} Complete: All stories in Feature {epicNum}.{featureNum} have been completed.
    
    Would you like to:
    1. Begin Feature {epicNum}.{featureNum + 1} with story {epicNum}.{featureNum + 1}.1
  ...
  - **CRITICAL**: NEVER automatically skip to another epic/feature or non-sequential story. The user MUST explicitly instruct which story to create if skipping the sequential order.

- **If no story files exist in `docs/stories/`:**
  - The next story is ALWAYS 1.1.1 (the first story of the first feature of the first epic).
  - If story 1.1.1 has unmet prerequisites, follow the same alert process as above.
- Announce the identified story to the user: "Identified next story for preparation: {epicNum}.{featureNum}.{storyNum} - {Story Title}".

### 2. Gather Core Story Requirements (from Epic and Feature)

- For the identified story, review its parent Epic (e.g., `epic-{epicNum}*.md` from the location identified in step 1.1).
- Also review the parent Feature (e.g., `feature-{epicNum}.{featureNum}*.md`).
- Extract: Exact Title, full Goal/User Story statement, initial list of Requirements, all Acceptance Criteria (ACs), and any predefined high-level Tasks.
- Keep a record of this original epic/feature-defined scope for later deviation analysis.
...
- Create a new story file: `{devStoryLocation}/{epicNum}.{featureNum}.{storyNum}.story.md` (using location from config).
- Use the Story Template to structure the file.
- Fill in:
  - Story `{EpicNum}.{FeatureNum}.{StoryNum}: {Short Title Copied from Feature File}`
  - `Parent Feature: {FeatureNum} - {Feature Title}`
  - `Status: Draft`
  - `Story` (User Story statement from Feature)
  - `Acceptance Criteria (ACs)` (from Feature, to be refined if needed based on context)
...
- Prepare content for the "Deviation Analysis" based on any conflicts between epic/feature requirements and architecture constraints.
...
- Ensure tasks align with both epic/feature requirements and architecture constraints
- Update status to "Draft"
- Save the story file to `{devStoryLocation}/{epicNum}.{featureNum}.{storyNum}.story.md` (using location from config)

### 9. Report Completion

Provide a summary to the user including:

- Story created: `{epicNum}.{featureNum}.{storyNum} - {Story Title}`
- Parent Feature: `{featureNum} - {Feature Title}`
- Status: Draft
- Key technical components included from architecture docs
- Any deviations or conflicts noted between epic/feature and architecture
